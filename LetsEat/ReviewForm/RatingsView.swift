...
//  LetsEat
//
//  Created by Ashraf Eltantawy on 08/09/2023.
//
...
private extension RatingsView{
    ...
    func draw(with image:UIImage,and fram :CGRect){
        image.draw(in: fram)
    }
}
...
extension RatingsView{
    ...
    func handel(with touch:UITouch){
    ...
    func updateRating (with value:Double){
        if(self.rating != value && value>=0 && value <= Double(self.totalStars)){
            self.rating = value
            setNeedsDisplay()
        }
    }
